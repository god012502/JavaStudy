...
/*
  for문
  while문과 같은 반복문
  초기값, 조건식, 증감식이 한줄에 있어 반복의 횟수를 명확히 알 수 있는 반복문
  따라서 반복의 횟수가 정해져 있는 경우에 주로 사용
  형식]
  		for(초기값; 조건식; 증감식){
  			실행문장;
  		}
*/
 public class E05For {
	...
	public static void main(String[] args) {
		...
		/*
		 	시나리오] for문을 이용하여 1~10까지의 정수 중 
		 	2의 배수의 합을 구하는 프로그램을 작성하시오.
		 	
		 	
		 */
		
		//방법1 : 1~10까지 10번 반복해서 if문을 통해 
	 	//2의 배수를 찾은 후 누적해서 더한다
		int total=0;
		for(int i=1;i<=10;i++) {
			if(i%2==0) {
				total+=i;
		...
		for(;;) {
			...
			/*
			  	무한루프는 기본적으로 에러가 발생
			  	그리고 해당 루프를 탈출하지 못한다면 아래에 있는 코드는 실행될 수 없는 
			  	코드가 되므로 "Unreachable Code"라고 에러가 발생하게 된다
		...
		/*
		 ...
		 라고 한다. 지역변수는 해당 지역을 벗어나면 즉시 소멸되는 특징을 가지고 있다.
		 */
		...
		/*
		 	...
		 	즉 아래 선언한 변수 i는 main메소드의 지역변수로 선언되었다.
		 */
		int i = 0;
		for( ;i<=5;i++) {
		...
		/*
		 	...
		 	즉 if문, while문 등의 제어문도 소도시로 생각하면 된다.
		 */
