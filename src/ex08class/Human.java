...
public class Human{
	...
	void walking() { //걷는다.
		...
		/*
		 	걷는 행동은 에너지를 1 차감시킨다.
		 	만약 에너지를 차감한 결과가 음수라면 해당 시나리오의 조건에
		 	위배되므로 0으로 고정시키는 코드가 조건문으로 포함되어 있다.
		 */
		energy--;
		if(energy<0) {
			energy=0;
	...
	void eating() { //먹다
		...
		if(energy>10) {
			energy=10;
			System.out.println("[eat]에너지는 10이 최대치 입니다.");
			}
		else {
			System.out.println("[eat]에너지가 2 감소하였습니다.");
		}
