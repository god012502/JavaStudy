...
class FruitSeller{
	...
	/*
	 	멤버변수의 경우 초기값이 없는 상태로 정의한 후 인스턴스 생성 후 초기화 할 수 있다.
	 	하지만 멤버상수는 인스턴스 생성과 상관없이 정의하는 시점에
	 	반드시 초기화 해야 한다. 즉 1000을 지우면 에러가 발생하게 된다.
	 */
	
	//과일 판매를 위한 멤버 메서드
	/*
	 	1. 구매자로부터 금액을 전달받는다
	 	   (메서드 호출 시 매개변수를 통해 받음)
	 	   
	 	2. 금액 / 단가 = 판매 될 사과의 갯수를 계산한다.
