...
public class PersonConstructor {
	...
	/*
	 	디폴트 생성자 
	 	해당 클래스에 생성자를 정의하지 않으면 아래와 같이 
	 	매개변수, 실행부가 없는 생성자가 컴파일러에 의해
	 	나동으로 삽입된다.
	 	지금까지 new 클래스명()과 같이 사용했던 문장이
	 	바로 디폴트 생성자를 호출하여 인스턴스를 생성하는 것이었다.
	 */
	//생성자 메서드
	...
	public PersonConstructor() {
		...
		//매개변수가 없어 값을 전달받을 수 없으므로 항상 고정된 값으로만 초기화된다.
		name = "이름없음";
		age=1;
		addr = "미상";
	...
	public PersonConstructor(String name) {
		...
		/*
		  	this는 멤버변수와 매개변수를 구분하기 위한 용도로 사용한다.
		  	클래스 자신, 즉 멤버를 가리킨다.
		 */
		this.name=name; 
		age=1;
	...
	void initialize(String name, int age, String addr) {
		...
		/*
		 	이 코드는 생성자 내에서만 사용할 수 있다.
		 	생성자는 인스턴스 생성시 딱 한번만 호출되고, 개발자가 임의로 호출 할 수 없다.
		 	하지만 멤버메서드는 언제든 호출할 수 있으므로 생성자와는 용도와 특성이 다르다.
