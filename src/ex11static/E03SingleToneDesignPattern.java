...
/*
 	...
 	즉, 하나의 메모리를 할당한 후 이를 프로그램 전체에서 공유해서 사용할 때 쓰는 패턴
 	
 	작성방법
 	1. 생성자의 접근지정자를 private으로 서언
 	2. 클래스 외부에서는 생성자에 접근할 수 없으므로 new를 통해 인스턴스 생성을 할 수 없다.
 	3. 클래스 내부에 멤버변수를 static으로 선언하여 인스턴스를 미리 생성해 놓는다.
 	4. getInstance()라는 유틸리티 메서드를 통해 참조값을 반환받아 사용한다.
 	5. 이 방법을 통해 인스턴스를 생성하면 메모리에 딱 하나만 만들어지게 
 	   되므로 메모리를 절약할 수 있다.
 */
...
class NoSingleTone{
	int instVar;
	/*
	 	생성자는 거의 대부분 public으로 선언한다.
	 	만약 public으로 선언하지 않으면 클래스 외부 혹은
	 	다른 패키지에서는 호출할 수 없기 때문에 new를 통해 
	 	인스턴스 생성 자체가 불가능하기 때문이다.
	 */
...
class SingleTone{
	...
	/*
	 	...
	 	JVM(자바가상머신)에 의해 프로그램이 시작될 때 미리 Method영역에
	 	로드되어 사용할 준비를 마치게 된다.
	 */
...
public class E03SingleToneDesignPattern {
	public static void main(String[] args) {
		/*
		 	일반적은 방식의 인스턴스 생성이므로, new 생성할 때 마다 새로운
		 	참조값을 할당 받게 된다.
		 */
