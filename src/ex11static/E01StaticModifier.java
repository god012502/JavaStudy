...
/*
 	...
 	- main() 메서드가 실행되기 전에 초기화되므로 main() 메서드 실행시 
 	  별도의 선언없이 변수를 즉시 사용할 수 있다.
 	  
 	접근방법
 	- 클래스 내부 방법 
 	  변수명으로 일반적인 멤버변수와 동일하게 접근 할 수 있다
 	  
 	- 클래스 외부
 	  1. 인스턴스 변수를 통해 접근한다.
 	  2. 인스턴스 생성없이 클래스명으로 직접 접근한다(권장사항)
 	
 	- 메서드 앞에 static을 붙이면 정적메서드가 된다.
 	  정적변수와 모든 특성이 동일하다.
 */
class MyStatic{
	/*
	 	멤버 변수
	 	인스턴스형 멤버변수는 반드시 클래스의 인스턴스를 생성한 후 
	 	참조변수를 통해 접근해야한다.
	 	
	 	즉, 일반적인 멤버변수는 인스턴스를 생성하는 시점에 메모리에
	 	로드된다.
	...
	/*
	 	정적 변수
	 	인스턴스를 생성할 필요없이 클래스명으로 바로 접근할 수 있다.
	 	JVM(자바가상머신)이 프로그램을 시작할 때 미리 메모리
	 	(Method 영역)에 로드된다.
	 */
	...
	/*
	 	정적 멤버메서드
	 	정적메서드 내부에서는 인스턴스형 멤버에는 접근 할 수 없다.
	 	정적 멤버만 접근 가능하다.
	 	(※ main 메서드에서는 static으로 선언된 
	 		메서드만 호출 할 수 있었던 것을 기억하자) 
	 */
	...
	public class E01StaticModifier {
		public static void main(String[] args) {
			/*
			 	일반적인 멤버변수에 접근하는 방법
			 	MyStatic클래스에 인스턴스를 생성한 후 참조변수를 통해 접근한다.
			 */
			...
			/*
			 	정적변수도 일반적인 멤버변수처럼 참조변수를 통해 
			 	접근할 수 있지만, Java에서는 권장하지 않는다.
	...
	////////////////////////////////////////////////////////////////////////////////////
			/*
			 	2개의 인스턴스를 생성한다.
			 	메모리(Heap영역)에는 ms1, ms2의 인스턴스가 각각 생성된다.
			 */
			...
			/*
			 	결과를 보듯 입력시에는 각각의 참조변수를 사용했지만
			 	결국 하나의 정적변수에 값을 입력한 결과가 되므로, 
			 	정적변수에 접근시에는 참조변수를 사용하지 말고, 클래스명을 사용하는 것이 좋다
			 */
			
			//이와 같이 작성하면 누가 보더라도 같은 변수임을 한눈에 알 수 있다
			MyStatic.staticVar=800;
			MyStatic.staticVar=900;
			
			/*
			 	정적 멤버 메서드에 접근 방법은 정적변수와 동일하다.
			...
			/*
			 	클래스명을 통한 접근은 정적메서드만 가능하다.
			 	인스턴스 머서드는 인스턴스 참조변수를 통해서만 접근할 수 있다
			 */
			//myStatic.instanceMethod();
			myStatic.staticMethod();		
			/*
			 	결론
			 	일반적인 인스턴스형 멤버는 인스턴스의 참조변수를 통해 접근한다.
