...
public class E03ReferenceYesNo {
	public static void main(String[] args) {
		/*
		 	...
		 	좌측은 부모 우측은 자식이므로 모두 실행가능한 문장이다.
		 */
		MyA refNum1=new MyB();
		MyA refNum2=new MyC();
		MyB refNum3=new MyC();
		...
		//MyB -> MyC 타입을 참조하므로 가능
		MyC refMyAddr2=refMyAddr1;
		
		//MyA  -> MyC 타입을 참조하므로 가능
		MyC refMyAddr3=refMyAddr1;
		
		//MyA -> MyC 타입을 참조하므로 가능
		MyA refId1=new MyC();
		//MyB -> MyA 타입을 참조하므로 불가능(자식은 부모를 참조할 수 없다)
//		MyB refId2=refId1;
		//MyC-> MyA 타입을 참조하므로 불가능
//		MyC refId3=refId1;
		
		/*
		 	refId1은 MyC인스턴스를 참조하지만,
		 	원래 MyA 타입이므로 MyC 타입이라 착각하면 안된다
		 */
		
		/*
		 	자식으로 부모 인스턴스를 참조하는 것은 기본적으로 불가능하다.
		 	하지만 다운캐스팅(강제 형변환)을 하면 참조가 가능해진다.
