...
/*
 	...
 	- 클래스에 정의하는 것이 마치 메서드를 정의하는 것과 비슷하다.
 	- 마지막에 세미콜론을 기술해야한다.
 	형식]
...
class Singer{
	...
	/*
	 	...
	 	반환해 준다. Java에서 정의하는 모든 클래스는 Object 클래스를 
	 	상속하므로 이와같이 오버라이딩 할 수 있다
	 */
	@Override 
...
/*
 	인터페이스 정의 
 	기능이 없는 추상메서드만 멤버로 정의할 수 있다
 	또한 메서드 앞에서 public abstract가 생략되어 있다.
 */
interface ISinger{
	public void dancing();
}
...
public class E02AnonymousClass {
	public static void main(String[] args) {
		...
		/*
	 	...
	 	이때 오버라이딩한 toString의 반환값이 출력된다.
	 	만약 toString을 삭제하면 @를 포함한 참조값이 출력되게 된다.
	 */
		...
		/*
		 	...
		 	이름이 없는 인스턴스이므로 부모클래스의 이름을 빌려서
		 	자식의 영역을 만든다고 생각하면 된다.
		 */
		Singer s3=new GirlGroup("아이린", "레드벨벳") {
			/*
			 	자식에서 확장한 멤버변수. 
			 	해당 변수는 익명클래스 내부에서만 사용할 수 있다. 
			 */
			boolean mic=true;
			@Override
			public String toString() {
		...
		/*
		 	...
		 	접근하고 싶다면 다운캐스팅(강제형변환)을 해야하지만
		 	익명클래스는 이름이 없기 때문에 다운캐스팅 자체가 불가능하다.
...
//		System.out.println(s3.mic); => 에러발생됨
		...
		/*
		 	인터페이스를 구현해서 익명클래스를 정의한다
		 	오버라이딩이 주된 목적이므로, 
		 	부모클래스에는 추상메서드만 정의되어 있어도 아무 상관이 없다.
		 */
