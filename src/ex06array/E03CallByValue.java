...
/*
 	...
 	메서드 호출과 관련된 개념으로 매개변수를 통해 값을 전달 할 때 메모리ㄴ의 '복사'를 통해 값이 전달된다.
...
public class E03CallByValue {
	...
	public static void callByValue(int fNum, int sNum) {
		...
		/*
		 	Swap(스왑) : 두개의 변수가 할당받은 값을 서로 교환하는 것을 말한다.
		 				교환시에는 임시변수(temp)가 하나 추가로 필요하다.
		 */
		int temp;
		temp=fNum;//frist100이 temp에 적용
		fNum=sNum;//second 200이 first에 적용
		sNum=temp;//first에 temp에 적용되었던 100이 first에 다시 적용
