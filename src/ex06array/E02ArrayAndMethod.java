...
public class E02ArrayAndMethod {
	public static void main(String[] args) {
		...
		/*
		 	...
		 	배열 자체가 전달되지 않는다. 아래 출력문에서는 배열 인스턴스의 참조값이 출력된다. 
		*/
		System.out.println("arr변수 : "+arr);
		
		/*
		 	arr은 참조값이지만 7인 기본자료형인 int이므로 
		 	값 자체가 전달된다.
		 	즉 복사가 되어 전달된다.
		 */
		...
		System.out.println();
		for(int i = 0; i<arr.length; i++) {
			System.out.println(arr[i]+" ");//8 9 10 11 12 출력
		}
	}
	/*
	 	매개변수로 전달된 배열의 참조값을 ar로 받게되므로 해당 메서드에서 동일한 배열을 참조할 수 있다
	 	즉 main메서드에서 생성한 배열과 동일한 배열을 참조하게 된다.
	 */
