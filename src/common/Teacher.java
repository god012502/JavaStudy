...
public class Teacher extends Person {
	...
	/*
	 	시나리오]
	 			Teacher 객체는 나이와 과목이 같으면 동일한 객체로 처리하도록
	...
	/*
	 	...
	 	정수형 멤버변수는 별도의 처리없이 사용하면 되고, 
	 	인스턴스형 멤버변수는 hashCode() 메서드를 통해 정수값으로 
	 	반환되는 참조값을 사용하면 된다.
	 */
	@Override
	public int hashCode() {
		/*
			방법 1
			정수형 변수와 인스턴스의 해시 코드값을 합산한 후 그대로 반환한다.
		 */
		...
		/*
		 	방법 2
		 	Objects 클래스의 정적메서드인 hash()를 사용한다.
	...
	/*
	 	...
	 	값이 같으면 true, 다르면 false를 반환하도록 오버라이딩 한다.
	 */
	@Override
	public boolean equals(Object obj) {
		...
		/*
		 	...
		 	subject는 String이므로 equals()를 통해 비교해야 한다.
		 	즉 우리가 정한 기준에 따라 멤버변수끼리 값이 동일한지
		 	비교하도록 오버라이딩 하면 된다.
		 */
		if((teacher.getAge()==super.getAge()) && teacher.subject.equals(this.subject)) {
			/*
			 	모든 내용이 일치하면 true을 반환한다.
			 	그러면 set에는 추가되지 않는다.
			 */
			return true;
		}
