...
/*
 	...
 	main메서드는 프로그램의 출발점의 역할만 담당하고, 
 	모든 기능은 핸들러 클래스에서 구현한다.
 */
...
public class FriendInfoHandler{
	...
	public void addFriend(int choice) {
		...
		System.out.print("이름"); iAddr=MyFriendInfoBook.scan.nextLine();
		...
		if(choice==1) {
			...
			/*
			 	인스턴스 참조값을 배열에 추가
			 	카운트용 변수를 후위증가 시켜 0번 인덱스에 먼저 입력된 후 1증가한다.
			*/
	...
	public void searchInfo() {
		...
		for(int i=0; i<numOfhighFriend; i++) {
			/*
			 	문자열 비교를 위한 메서드 중 comareTo()를 사용해서
			 	검색 기능 구현. 
			 	"문자열1.compareTo(문자열2)" 형식으로 사용하고
			 	일치하는 경우 0을 반환한다.
			 */
	...
	public void deleteInfo() {
		...
		/*
		 	...
		 	배열의 인덱스는 0부터 시작이므로 초기값은 -1로 설정한다.
		 */
		...
		if(deleteIndexHigh!=-1) {
			...
			for(int i=deleteIndexHigh; i<numOfhighFriend; i++) {
				/*
				 	삭제된 인스턴스의 인덱스부터 마지막까지 반복하면서
				 	뒷쪽의 인스턴스를 한칸씩 앞으로 이동한다.
				 	배열 가운데 null값이 입력되어 있으면 정보출력 시 
