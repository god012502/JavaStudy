...
class MyClass{
	...
	/*
	 	...
	 	재정의 한다. 인스턴스가 가진 멤버변수의 값에 대한 비교를 통해
	 	동일 여부를 판단한다. equals() 메서드의 매개변수 타입은 
	 	모든 인스턴스를 대상으로 하므로 Object형으로 정의되어 있다.
	 */
	@Override
	public boolean equals(Object obj) {
		/*
		 	...
		 	다운캐스팅 이후에 비교를 해야한다.
		 	이때 전달된 인스턴스가 MyClass형이 아니라면 비교의 대상이 될 수 
		...
		if(obj instanceof MyClass) {
			...
			/*
			 	매개 변수로 전달된 인스턴스가 MyClass 타입이 아니라면
			 	비교의 대상이 될수 없으므로 false를 반환한다.
			 */
...
public class E04Object3Equals {
	public static void main(String[] args) {
		...
		System.out.println("[두 객체를 비교 연산자를 통해 비교]");
		//참조값에 대한 비교이므로 '다르다'라는 결과가 출력됨
		if(mc1==mc2) {
		...
		/*
		 	...
		 	단, 오버라이딩 해놓은 equals()를 삭제한 후 비교하면 다르다는 결과가 
		 	나오게 된다. 즉 우리가 직접 정의한 클래스라면 반드시 오버라이딩이 필요하다
		 */
		System.out.println("[두 객체를 equals() 메소드로 비교]");
		System.out.println(mc1.equals(mc2)?"같다":"다르다");//같다

		System.out.println("[두 객체를 toString() 메소드로 비교]");
