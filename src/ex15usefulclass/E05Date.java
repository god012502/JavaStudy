...
/*
 	Calender 클래스를 통해 얻은 인스턴스를 초(Sec)로 변환한 후 매개변수로 전달된 날짜서식으로
 	변환한 문자열을 반환한다.
 */
public class E05Date {
	public static String myDateFormat(Calendar c,String formatStr) {
		/*
		서버의 표준시인 1970-01-01부터 현재까지의 시간을 초 단위로 
	...
	*/
	...
	public static void main(String[] args) {
		/*
		 	Celender 클래스는 Java의 대표적인 싱글톤 객체이므로 
		 	new를 사용하는 대신 유틸리티 메서드인 getInstance()를 통해
		 	참조값을 얻어온다.
		 */
		 ...
		 */
		String nowStr1=myDateFormat(calender, "yyyy-MM-dd HH:mm:ss");
		System.out.println("현재1:"+nowStr1);
		// 그 외 여러가지 서식 문자가 존재함
		String nowStr2=myDateFormat(calender, "오늘은 M월의 W번째주, d번째날, F번째 E요일");
		System.out.println("현재2:"+nowStr2);
		
		/*
		 	현재 날짜를 기준으로 과거.미래를 구하기 위해 
		 	add 메서드를 사용한다.
		 	두번째 음수가 음수면 과거의 날짜를 구할 수 있다.
		 */
		
		calender.add(Calendar.DATE,3); //3일 후 
		calender.add(Calendar.MONTH,2);//2개월 후 
		calender.add(Calendar.YEAR,1);//내년 (1년 후 )
		
		/*
		 	앞에서 add를 3번 호출했으므로 결과는 아래와 같이 1년 2개월 3일후가 출력된다.
		 */
		String nowStr3 = myDateFormat(calender, "yyyy년 MM월 dd일");
		System.out.println("1년 2개월 3일후:"+nowStr3);
		
		/*
		 	올해 크리스마스까지 남은 날짜를 계산
		 	여기서는 두가지 서식의 날짜를 파싱하여 구한 날짜 인스턴스로 계산한다.
		 */
	...
	public static long myDiffDate(String today, String future) {
		...
		/*
		 	...
		 	따라서 각 날짜의 서식을 명확히 지정해야만 날짜를 인식하여 연산 할 수 있다.
		 */
		try {
			//각 서식의 날짜를 파싱해서 Date인스턴스로 변환한다.
