...
/*
 ...
 디폴트 생성자는 자동으로 삽입되고, 자식쪽에 super()도
 자동으로 삽입될 수 있으므로 생략 가능한 코드가 존재한다.
...
class BB extends AA{ 
	public BB() { //생략가능
 	...
 	부모클래스를 자식클래스를 상속할 때 extends를 사용한다.
 */
...
class Man{
	...
	//각 생성자는 print()를 통해 호출여부를 확인 할 수 있다.
...
class BusinessMan extends Man{
	...
	//생성자 : 멤버변수는 2개지만, 매개변수로 3개로 선언
	public BusinessMan(String name, String comany, String position) {
		
		/*
		 	상속받은 자식클래스는 부모클래스의 생성자를 호출하여 
		 	부모인스턴스가 메모리에 먼저 생성되도록 해야한다.
		 	
		 	super()는 부모클래스의 생성자를 호출해준다. 
		 	만약 super()를 기술하지 않으면 컴파일러에 의해 
		 	자동으로 삽입되어 디폴트 생성자를 호출하게 된다.
		 */
	...
	public void tellYourInfo() {
		...
		/*
		 	...
		 	상속받은 자식 클래스에 접근할 수 없다.
		 	
		 	private멤버는 무조건 자신의 클래스 내부에서만 접근할 수 있다
		 */
		
		//System.out.println("이름은 "+name+"입니다");
		
		/*
		 	직접 접근이 안되므로 public으로 선언된 메서드를 통해
		 	간접적으로 접근해야 한다.
		 */
//		tellYourName();
	}
