...
/*
 	static(정적) 형식의 멤버도 상속이 되고, 하위클래스에서 접근 시 
 	멤버 변수의 이름만으로 접근이 가능하다
 	단, 클래스 외부에서 접근할 때는 인스턴스 생성없이 클래스명을 통해 
 	접근할 수 있다.
...
class Adder{
	...
	/*
	 	static으로 선언던 멤버변수는 실제로는 클래스 외부(Method영역)에
...
/*
 	...
 	따라서 자식 클래스에 작성된 모든 코드는 문제없이 실행된다.
 */
class AdderFriend extends Adder{
	public void frienAdd(int num) {
		//멤버변수명 만으로 접근 가능
		val+=num;
		//클래스명 통해서도 접근 가능
		Adder.val+=num;
...
public class E03StaticInheritance {
	...
	/*
	 	앞에서 호출한 메서드는 모든 메서드 영역에 
	 	생성된 정적변수 val에 더해지게 되므로 
	 	출력결과는 20이 된다.
	 */
	ad.add(1);
	adFriend.frienAdd(3);
