...
class Account{
	...
	/*
	 	이와 같이 개발자가 생성자를 직접 정의하면 디폴트 생성자를 
	 	추가되지 않는다. 생성자를 정의하지 않는 경우에만 자동으로 추가된다.
	 */
	...
	//private 멤버변수를 외부로 반환 할 때 사용하는 메서드(getter)
...
class SavingAccount extends Account{
	...
	public SavingAccount(int initVal) {
		/*
		 	이 문장을 삭제하면 에러가 발생하게 된다.
		 	삭제하는 경우 super()라는 문장,
		 	즉, 매개변수가 없는 디폴트 생성자를 호출하는 문장이 
	...
	public void showAccountMoney() {
		/*
		 	잔고확인을 위한 멤버메서드로 부모의 멤버인 money에
		 	직접 접근 할 수 없으므로 아래와 같이 getter메서드로
		 	반환받아 출력해야 한다.
		 */
...
public class E02PrivateInheritanceMain {
	public static void main(String[] args) {
		...
		/*
		 	...
		 	하지만 해당 인스턴스 변수를 통해서도 money에 접근 할 수없다.
		 	현재의 위치가 외부 클래시이기 때문이다.
