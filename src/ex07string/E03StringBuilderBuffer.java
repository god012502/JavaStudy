...
/*
  	StringBuffer 클래스
  	String클래스는 기존 문자열에 새로운 문자열을 추가하면 새롭게 생성된 메모리에 문자열을 저장한다.
  	기존 메모리가 소멸되고 새로운 메모리가 생성되는 낭비를 막기위해, 문자열의 변경이 많은 경우 StringBuffer를
  	사용하는 것이 좋다. 이 클래스는 기존 메모리에 문자열을 추가(혹은 변경)하는 방식으로 동작한다.
...
public class E03StringBuilderBuffer {
	...
	public static void main(String[] args) {
		...
		//더블쿼테이션을 통해 정의했으므로 동일한 잠조값을 가진다.
		...
		/*
		  	String은 문자열의 변경이 있는경우
		 	기존 메모리를 소멸하고 새로운 메모리를 할당한다.
		 */
		str1=str1+"&Spring";
		if(str1==str2)
		...
		/*
		 	StringBuffer는 문자열 저장을 위해 기본 메모리를 
		 	16으로 할당한다. 차후 저장공간이 부족해지면 자동으로 확장한다.
		 */
		...
		System.out.println("금일은 StringBuffer 학습중..!!");
		System.out.println("buf = "+buf);
		System.out.println("저장된 문자열 크기 : "+buf.length());
		System.out.println("기본 버퍼크기 : "+buf.capacity());
	}
