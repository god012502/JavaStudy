...
/*
 	...
 	3. 예외발생 지점에서 if문으로 감지한 후 
 	   예외 인스턴스를 생성 및 throw 한다.
...
public class Ex06DeveloperDefine2 {
	public static void main(String[] args) {
		...
		try {
			/*
			 	아래 메서드에서 AgeErrorException 예외 신스턴스를 
			 	throw 하면 여기에서 예외처리한다.
	...
	public static int readAge() throws AgeErrorException{
		...
		try {
			/*
			 	나이 입력시 문자를 잘못 입력하는 경우에는 
			 	Java에서 미리 정의해 둔 예외처피 클래스를 사용하면 된다.
			 */
			inputAge=sc.nextInt();
		}
		...
		/*
		 	...
		 	이때는 if문을 통해 판단해야 한다.
		 	개발자가 정의한 예외 인스턴스를 성상한 후 직접 throw한다.
		 */
		if(inputAge<0) {
