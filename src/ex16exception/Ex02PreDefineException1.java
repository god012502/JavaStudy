...
public class Ex02PreDefineException1 {
	public static void main(String[] args) {
		...
		/*
		  	...
		  	JVM(자바가상머신)은 해당 예외에 관련된 인스턴스를 만들어 프로그램쪽으로 전달한다.
		  	이때 catch절에서 예외 인스턴스를 잡아 주는 것을
		  	"예외처리"라고 한다
		...
		try {
			...
			/*
			 	예외가 발생하는 지점.
			 	해당 코드가 실행 될 때 예외가 발생하게 되고 JVM이
			 	예외 인스턴스를 생성해서 프로그램으로 전달한다.
			 */
			intArr[2]=300;
			...
			//예외메세지 1 : 개발자가 임의로 메세지를 출력
			System.out.println("예외가 발생했어요");
			//메세지 2:간략한 설명을 출력
			System.out.println("예외메세지:"+e.getMessage());
			//메세지 3 : 예외에 대한 디테일한 설명을 출력(가장 많이 사용됨)
			e.printStackTrace();
		}
		/*
		 	예외처리를 통해 실행의 흐름이 중단되지 않으므로
		 	아래 문장은 정상적으로 출력된다.
		 */
