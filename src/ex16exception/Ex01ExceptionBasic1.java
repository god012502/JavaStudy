...
//			throws IOException{
		/*
		 	Syntax에러(구문에러)
		 	실행자체가 되지 않은 코드로 오류를 수정해야만 정상적으로 실행된다.
...
//		{}
		...
		/*
		 	외부자원을 사용 할 때 발생되는 예외
		 	
		 	처리방법 1]
		 			"예외 던지기"를 수행한다.
		 			main메서드에 throws IOException을 기술하면
		 			해당 메서드 내에서 발생되는 예외를 무시하겠다는 의미가 된다.
		 			
		 	처리방법 2]
		 			예외를 발생한 지점을 try ~ catch문으로 직접 감싸서 처리한다.
		 */
