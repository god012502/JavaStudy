...
/*
 	...
 	- 스택은 입구가 하나만 있는 컵과 같은 형태이며, 
 	  '선입후출'의 특성을 가진다.
 	  이 메모리는 CPU가 생성 및 소멸을 주관한다.
 */
public class E06LocalVariable {
	public static void main(String[] args) {
		...
		/*
		 	...
		 	선언한다. 단, if문과 else문의 블럭을 서로 다른 지역이므로 동일한
		 	이름의 변수를 선언할 수 있다.
		 */

		//int num=9; 이 부분이 활성화되면 아래에서 에러발생됨
		/*
		...
		if(scope) {
			int num = 1;
			num++;
			System.out.println("첫 번재 if문 지역 : "+num);
		}
		else {
			int num = 5;
			System.out.println("첫 번째 if문의 지역 : "+num);
		}
		/*
	...
	/*
	 	main에서 선언한 변수 num은 메모리에 아직 상주하고 있디만
	 	해당 메서드는 크기가 동일한 서로 다른 지역에 정의되었으므로 
	 	같은 이름은 변수를 선언할 수 있다.
	 	앞의 main에서 if~false문의 블럭도 같은 개념으로 변수를 선언 한 것이다.
	 */
	
	static void simpleFunc() {
		int num = 1000;
		System.out.println("simle 메소드 지역 : "+num);
	}
}
