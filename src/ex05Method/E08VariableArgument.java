...
/*
 	...
 	메서드 오버로딩을 대체할 수 있는 문법
 	하나의 메서드로 여러가지 호출형식을 대체할 수 있다.
 	형식]
 		메서드명(자료형 ... 매개변수명){
 			실행문장;
 		}
 	-필요에 따라 매개변수를 가변적으로 저정할 수 있는 문법
...
//	}
	...
	/*
	 	...
	 	또한 메서드 내에서는 배열로 반환되어 이를 통해 반복문으로 접근할 수 있다.
	 */
	static int getTotal(int ...param) {
		int total = 0;
...
//		+getTotal(10,20,30,40,50,60));
		...
		/*
		 	매개변수의 타입이 double이 포함되어 있으므로
		 	호출할 수가 없다. 만약 아래와 같이 실수를 모두 처리하려면 
		 	가변인자의 타입을 double로 선언하면 된다.
		 */
