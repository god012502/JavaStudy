...
public class E02SilsuType {
	...
	방법1 : float를 long으로 강제 형변환 후 연산
	방법2 : 계산결과 전체를 long으로 강제 형변환
	
	- 실수형의 기본은 double
	- 소수점이 있는 데이터를 컴파일러는 무조건 double로 인식
	- 단, 소수점이 없는 데이터는 float에 대입할 수 있음
	- 소수점이 있는 데이터를 float에 대입하려면 접미사(F 혹은 f)를 필히 붙여야 함
	 */
	public static void main(String[] args) {
		
		long long1 = 100;
		float float1=200;
		//정수와 실수의 연산은 실수로 자동 형변환 된다.
		...
		//강제 형변환 후 대입할 수 있으나 (권장x)
		float f4 = (float)3.14;
		System.out.println("f4 ="+f4);
		
		//접미사를 통해 float형 자료임을 명시 (권장사항)
		float f5 = 3.14F;
