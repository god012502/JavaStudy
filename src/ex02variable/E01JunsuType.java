...
public class E01JunsuType {
	/*
		...
		- CPU는 정수의 연산인 경우 int형에 최적화 되어 있기 때문이다.
	...
	public static void main(String[] args) {
		...
		/*
		 자바는 정수형 상수를 기본적으로 int형으로 간주하기 때문에 아래 정수를 메모리에 로드하는 순간 에러가 발생한다.
		 이때 접미사(L 혹은 l)을 붙여서 int형의 자료가 아닌 long형의 자료임을 표시해야 한다.
		 이와 같이 입력되는 숫자도 자료형을 기반으로 메모리에 로드되는데 이를 가르켜 리터럴(Literal)이라고 한다.
		 */		
