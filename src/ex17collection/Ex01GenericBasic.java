...
public class Ex01GenericBasic {
	public static void main(String[] args) {
		...
		orange1.showInfo();
		/*
		 	상자1은 오렌지 전용박스이므로 다른과일(즉 인스턴스)를
		 	저장 할 수 없다. 컴파일 에러가 발생하므로 실행 자체가
		 	불가능하다. 즉 자료형에는 안전하지만, 구현에는 불편함이 있다.
		 	다른 인스턴스를 저장하기 위해서는 또 다른 Box클래스를 생성해야 한다.
		 */
		
		/*
		 	아래와 같이 Orange가 아닌 인스턴스를 저장하면
		 	즉시 컴파일 에러가 발생하여 실행할 수 없은 코드가 된다.
		 	즉 자료형에는 안전하다고 볼 수 있다.
		 */
		//oBox1.store("당도가 20인 오렌지"); 에러발생
		...
		orange3.showInfo();
		/*
		 	하지만 오렌지 같은 String을 저장하고,
		 	이를 꺼내서 다운 캐스팅하는 과정에서 예외가 발생된다.
		 	즉 런타임 에러가 발생하게 되므로 실행전에는 오류를 
		 	예측하기 힘든 단점이 있다.
		 */
		FruitBox fBox2=new FruitBox("당도가 30인 오렌지");
		Orange orange4=(Orange)fBox2.pullOut();//런타임 오류
		orange4.showInfo();
	}
}
