...
//		Vector<String> list=new Vector<String>();
		...
		/*
		 	인스턴스 저장 add()
		 		add(인스턴스)
		 		순차적으로 저장하면서 인덱스는 0부터 자동으로 부여된다.
		 		
		 		add(인덱스, 인스턴스)
		 		데이터 저장 시 인덱스를 직접 부여한다.
		 		단, 인덱스를 건너뛰면 에러가 발생되므로 주의해야 한다.
		 		저장하려는 인덱스에 이미 저장된 데이터가 있다면 삽입(끼워넣기)이 된다.
		 */
		list.add("소녀시대");
		list.add("빅뱅");
		list.add("트와이스");
...
//		list.add(5,"소방차ㅋㅋ"); 에러발생됨
		...
		/*
		 	방법 2 
		 	확장 for문의 경우 인덱스 없이 전체 원소를 반복해주므로
		 	좀 더 간단한 표현이 가능하다.
		 	따라서 가장 많이 사용된다.
		 */
		System.out.println("\n[확장for문 사용]");
		...
		/*
		 	...
		 	2. hasNext()로 인출한 인스턴스가 있는지 검사한다.
		 	   만약 없다면 false를 반환한다.
		...
		/*
		 	포함여부확인
		 	특정 인스턴스가 저장되어 있는지 확인 할 때 사용
		 	존재한다면 true를 반환한다.
		 */
		System.out.println(list.contains("빅뱅")?"빅뱅 있음":"빅뱅 없음");//있음
		System.out.println(list.contains("블랙핑크")?"블랙핑크 있음":"블랙핑크 없음");//없음
		
		/*
		  	삭제 방법1
		  	인덱스로 삭제
		  	인덱스를 통해 삭제한 후 완료되면 해당 인스턴스의 참조값을 반환한다.
		  	삭제 후에는 전체적인 인덱싱이 자동으로 진행된다.
		 */
		...
		/*
		 	방법2
		 	방법1과 동일하게 인덱스로 삭제한다.
		 	단, indexOf로 인스턴스의 인덱스를 찾은 후 이 값을 사용한다.
		 */
